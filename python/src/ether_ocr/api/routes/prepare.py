...
@router.post(
    ...
    description=(
        "Upload a PDF (with text layer) or UTF-8 text file. "
        "Extracts text with Poppler if needed, cleans layout artifacts, "
        "and validates RAG compatibility. Returns cleaned text with metadata."
    ),
)
...
@router.post(
    ...
    description=(
        "Upload a UTF-8 text file to validate it against RAG ingestion rules. "
        "Returns whether the text is valid and lists any issues found "
        "(Markdown, HTML, control characters, etc.)."
    ),
)
