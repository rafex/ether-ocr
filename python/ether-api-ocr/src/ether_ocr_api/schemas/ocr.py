...
class OcrMetadata(BaseModel):
    ...
    ocr_used: bool = Field(
        description="Whether OCR was applied (vs direct text extraction)"
    )
    method: str = Field(
        description="Method used: 'poppler', 'tesseract', or 'passthrough'"
    )
...
class BatchFileResult(BaseModel):
    ...
    metadata: Optional[OcrMetadata] = Field(
        default=None, description="Processing metadata (null on error)"
    )
