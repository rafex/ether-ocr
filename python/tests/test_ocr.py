"""Tests for ether_ocr.ocr module — unit tests without requiring Tesseract."""
...
from ether_ocr.ocr import OcrError, OcrResult, has_text_layer, ocr_image, ocr_pdf_scanned
...
class TestOcrImage(unittest.TestCase):
    ...
    @patch("ether_ocr.ocr.shutil.which", return_value="/usr/bin/tesseract")
    @patch("ether_ocr.ocr.subprocess.run")
    @patch("ether_ocr.ocr.tempfile.TemporaryDirectory")
    def test_ocr_success(self, mock_tmpdir, mock_run, mock_which):
        ...
        with patch("pathlib.Path.exists", return_value=True):
            with patch.object(Path, "read_text", return_value="Texto extraído de prueba."):
    ...
    @patch("ether_ocr.ocr.shutil.which", return_value=None)
...
class TestOcrPdfScanned(unittest.TestCase):
    ...
    def test_ocr_pdf_with_images(self, mock_tmpdir, mock_which):
        ...
        with patch("pdf2image.convert_from_path", return_value=[mock_image]):
            ...
            with patch("ether_ocr.ocr.ocr_image") as mock_ocr_image:
...
class TestHasTextLayer(unittest.TestCase):
    ...
    @patch("ether_ocr.ocr.shutil.which", return_value="/usr/bin/pdftotext")
    @patch("ether_ocr.ocr.subprocess.run")
    ...
    def test_meaningful_text_returns_true(self, mock_run, mock_which):
        mock_run.return_value = MagicMock(returncode=0)
        with patch.object(Path, "read_text", return_value="Este es un texto largo con suficiente contenido."):
            result = has_text_layer(Path("/tmp/test.pdf"))
