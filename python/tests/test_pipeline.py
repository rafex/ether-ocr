"""Tests for ether_ocr.pipeline module — unit tests without requiring OCR deps."""
...
class TestOcrPipeline(unittest.TestCase):
    ...
    @patch("ether_ocr.pipeline.ocr_image")
    def test_image_input_uses_ocr(self, mock_ocr_image):
        from ether_ocr.ocr import OcrResult
    ...
    @patch("ether_ocr.pipeline.extract_pdf_text")
    ...
    @patch("ether_ocr.pipeline.extract_pdf_text", return_value="")
    @patch("ether_ocr.pipeline.has_text_layer", return_value=False)
    @patch("ether_ocr.pipeline.ocr_pdf_scanned")
    def test_pdf_without_text_falls_back_to_ocr(self, mock_ocr, mock_has_text, mock_extract):
        from ether_ocr.ocr import OcrResult
    ...
    def test_plain_text_passes(self):
        ...
        input_file.write_text("Texto completamente plano.\n\nSegundo párrafo.", encoding="utf-8")
        output = self.temp_dir / "output.txt"
